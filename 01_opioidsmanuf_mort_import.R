...
## Title: Mortality Data Import
## File:  01_opioidsmanuf_mort_import.R
## Description: Imports 1999-2017 mortality data from NCHS MCOD Dataset. Next, 
##  cleans up data and filters data to create drug overdose deaths dataset
...
mort.1999.2002_prep <-  mort.1999.2002 %>%
  mutate(statecountyfips = ifelse(nchar(fipsctyr)==4,
                                  paste0("0", fipsctyr),
                                  fipsctyr),
         marstat = ifelse(marstat == 1, "S",
                   ifelse(marstat == 2, "M", 
                   ifelse(marstat == 3, "W",
                   ifelse(marstat == 4, "D",
                   ifelse(marstat == 8, "N",
                   ifelse(marstat == 9, "U", NA)))))))
